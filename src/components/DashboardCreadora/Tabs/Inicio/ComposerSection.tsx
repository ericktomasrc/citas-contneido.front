...
export const ComposerSection = ({ className = '', onPublicar, retosSugeridos = [], retosActivos = false }: ComposerSectionProps) => {
  ...
  // Resetear opciones de suscriptores al cambiar a público
  useEffect(() => {
  ...
  const handlePublicar = () => {
    // Caso 1: Solo descripción (sin archivos) → siempre público
    ...
    // Caso 2: Con archivos → validar según audiencia
    if (archivosSeleccionados.length === 0) return;
    
    // Validaciones para suscriptores
    if (audiencia === 'suscriptores') {
      // Validar contenido sugerido
      if (contenidoSugeridoActivado && !ideaSeleccionada) {
        setToastMessage('⚠️ Selecciona una idea de contenido sugerido');
        setShowToast(true);
      ...
      if (premiumActivado && (!precioPremium || parseFloat(precioPremium) <= 0)) {
        setToastMessage('⚠️ Ingresa un precio válido para contenido premium');
        setShowToast(true);
        setTimeout(() => setShowToast(false), 3000);
        return;
      }
    ...
    // Si tiene contenido sugerido activo, agregar el texto del pedido
    if (contenidoSugeridoActivado && ideaSeleccionada) {
  ...
  // Validar si puede publicar - ahora siempre true si hay contenido
  const canPublish = archivosSeleccionados.length > 0 
    ? true // Siempre habilitado, validación en handlePublicar
    : nuevoPost.trim() !== ''; // Solo descripción

  return (
    <>
      <div className={`bg-white rounded-xl shadow-sm border border-slate-100/80 overflow-hidden ${className}`}>
        {/* Badge "Pedido por la comunidad" - SOLO SI contenidoSugeridoActivado está activo */}
            ...
            <div className="flex items-center gap-3 mb-2">
              ...
              >
                <div className={`w-3.5 h-3.5 rounded-full border-2 flex items-center justify-center transition-all ${
                  audiencia === 'publico' ? 'border-emerald-500' : 'border-slate-300 group-hover:border-slate-400'
                }`}>
                  {audiencia === 'publico' && <div className="w-1.5 h-1.5 rounded-full bg-emerald-500" />}
                </div>
                <span className={`text-[10px] font-medium transition-colors ${
                  audiencia === 'publico' ? 'text-emerald-600' : 'text-slate-500 group-hover:text-slate-600'
                }`}>Público</span>
              </button>
              ...
              >
                <div className={`w-3.5 h-3.5 rounded-full border-2 flex items-center justify-center transition-all ${
                  audiencia === 'suscriptores' ? 'border-rose-500' : 'border-slate-300 group-hover:border-slate-400'
                }`}>
                  {audiencia === 'suscriptores' && <div className="w-1.5 h-1.5 rounded-full bg-rose-500" />}
                </div>
                <span className={`text-[10px] font-medium transition-colors ${
                  audiencia === 'suscriptores' ? 'text-rose-600' : 'text-slate-500 group-hover:text-slate-600'
                }`}>Solo suscriptores</span>
              </button>
            </div>

            {/* Línea 2: Checkboxes + Botones */}
            <div className="flex items-center gap-2 flex-wrap">
              {/* Texto bonito cuando es Público */}
              {audiencia === 'publico' && (
                <p className="text-[10px] text-slate-400 italic">✨ Comparte algo increíble con toda tu audiencia</p>
              )}

              {/* Checkboxes - SOLO si audiencia es suscriptores */}
              {audiencia === 'suscriptores' && (
                <>
                  {/* Contenido sugerido - desactivado si retosActivos es false */}
                  <button
                  ...
                  >
                    <div className={`w-3 h-3 rounded border flex items-center justify-center transition-all ${
                      contenidoSugeridoActivado ? 'bg-rose-400 border-rose-400' : 'border-slate-300 group-hover:border-slate-400'
                    }`}>
                  ...
                  {/* Premium - siempre activo cuando audiencia es suscriptores */}
                  <button
                    onClick={() => setPremiumActivado(!premiumActivado)}
                    className="flex items-center gap-1 group"
                  >
                    <div className={`w-3 h-3 rounded border flex items-center justify-center transition-all ${
                      premiumActivado ? 'bg-emerald-600 border-emerald-600' : 'border-slate-300 group-hover:border-slate-400'
                    }`}>
              ...
              {/* Flex spacer - empuja los botones a la derecha */}
              <div className="flex-1" />
              ...
              <button 
                ...
                disabled={!canPublish} 
                className={`px-4 py-2 rounded-xl text-[10px] font-bold transition-all flex-shrink-0 shadow-sm ${
                  !canPublish 
        ...
        {/* Footer simple - SOLO CUANDO NO HAY ARCHIVOS */}
        {archivosSeleccionados.length === 0 && (
          <div className="px-3.5 pb-3 flex items-center gap-2">
            ...
            {/* Indicador "Público" siempre */}
            <div className="flex items-center gap-1 text-[8px] text-slate-400 flex-shrink-0">
              <Globe className="w-2.5 h-2.5" />Público
            </div>

            {/* Botón Publicar - con gradiente verde */}
            <button 
              onClick={handlePublicar} 
      ...
      {showToast && createPortal(
        <div className="fixed top-4 right-4 z-[99999] animate-in slide-in-from-top-2 fade-in duration-300">
          <div className="bg-gradient-to-r from-red-50 to-orange-50 rounded-xl shadow-2xl border-2 border-red-200 px-4 py-3 flex items-center gap-3 min-w-[320px]">
            <div className="w-10 h-10 rounded-full bg-red-100 flex items-center justify-center flex-shrink-0">
              <span className="text-xl">⚠️</span>
            </div>
            <div className="flex-1">
              <p className="text-[12px] font-bold text-red-600">{toastMessage}</p>
            </div>
            <button 
              onClick={() => setShowToast(false)}
              className="w-6 h-6 rounded-full hover:bg-red-100 flex items-center justify-center transition-all flex-shrink-0"
            >
