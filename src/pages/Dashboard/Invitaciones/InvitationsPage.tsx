...
interface Invitation {
 ...
 const InvitationsPageContent = () => {
  ...
  const tabs = [
    { id: 'aceptada' as const, label: 'Aceptadas', icon: Check, count: invitations.filter(i => i.status === 'aceptada').length },
    { id: 'pendiente' as const, label: 'Pendientes', icon: Clock, count: invitations.filter(i => i.status === 'pendiente').length },
    { id: 'rechazada' as const, label: 'Rechazadas', icon: X, count: invitations.filter(i => i.status === 'rechazada').length },
  ];
              ...
              {filteredInvitations.map((inv) => {
                ...
                return (
                  <div
                    key={inv.id}
                    className={`
                      ${config.bg} border-2 ${config.border} rounded-2xl p-4 shadow-sm hover:shadow-md transition relative
                    `}
                  >
                    ...
                    <div className="flex flex-col items-center mt-8 mb-3">
                      ...
                      {/* Nombre */}
                      <h3 className="text-base font-bold text-slate-900 mb-2 text-center truncate w-full px-2">
                        {inv.userName}
                      </h3>

                      {/* Días transcurridos */}
                      {(inv.status === 'pendiente' || inv.status === 'rechazada') && (
                        <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-white/70 text-slate-600 text-xs rounded-full mb-2">
                    ...
                    {inv.status === 'aceptada' ? (
                      <div className="space-y-2">
                        ...
                        <a
                          ...
                          className="flex items-center gap-2 p-2 bg-white hover:bg-slate-50 rounded-lg transition group"
                        >
                          <Phone className="w-4 h-4 text-green-600 flex-shrink-0" />
                          <span className="text-xs font-medium text-slate-900 truncate">{inv.whatsapp}</span>
                        </a>
                        ...
                        <div className="flex items-center justify-center gap-2">
                          <a
                            ...
                            target="_blank"
                            rel="noopener noreferrer"
                            className="w-9 h-9 flex items-center justify-center bg-white hover:bg-slate-50 rounded-lg transition group"
                            title={inv.telegram}
                          >
          ...
          >
            <div className="border-b border-slate-200 p-6">
              <div className="flex items-center gap-3">
                ...
                />
                <div>
                  <h3 className="text-lg font-bold text-slate-900">{selectedInvitation.userName}</h3>
                  <p className="text-sm text-slate-600">Enviar mensaje</p>
                </div>
              </div>
            </div>
            ...
            <div className="p-6 space-y-4">
                ...
                <div className="flex items-center justify-between">
                  ...
                  {selectedInvitation.mensajesEnviados > 0 && (
                    <div className="text-right">
                      <p className="text-xs text-slate-600">Tu saldo</p>
                      <p className="text-lg font-bold text-slate-900">{saldoUsuario} coins</p>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div>
                ...
                />
                <p className="text-xs text-slate-500 mt-1 text-right">{messageText.length}/500</p>
              </div>
            ...
            <div className="border-t border-slate-200 p-6 flex gap-3">
              <button
                onClick={() => setShowMessageModal(false)}
                className="flex-1 px-4 py-3 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-lg font-medium transition"
              >
                Cancelar
              </button>
              ...
              {selectedInvitation.mensajesEnviados > 0 && saldoUsuario < costoPorMensaje ? (
                ...
                <button
                  ...
                  className="flex-1 px-4 py-3 bg-blue-600 hover:bg-blue-700 disabled:bg-slate-300 disabled:cursor-not-allowed text-white rounded-lg font-medium transition flex items-center justify-center gap-2"
                >
