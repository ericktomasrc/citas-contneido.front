...
export const InvitationsPage = () => {
  ...
  const [filterStatus, setFilterStatus] = useState<'todas' | 'aceptada' | 'pendiente' | 'rechazada'>('todas');
  ...
  const filteredInvitations = filterStatus === 'todas' 
    ? invitations 
        ...
        <div className="p-6 lg:p-8">
          ...
          <div className="mb-6 flex gap-2 overflow-x-auto pb-2">
            {[
              ...
              { id: 'aceptada', label: 'Aceptadas', count: invitations.filter(i => i.status === 'aceptada').length },
              { id: 'pendiente', label: 'Pendientes', count: invitations.filter(i => i.status === 'pendiente').length },
              { id: 'rechazada', label: 'Rechazadas', count: invitations.filter(i => i.status === 'rechazada').length },
            ].map((filter) => (
              <button
                key={filter.id}
                onClick={() => setFilterStatus(filter.id as any)}
                className={`
                  px-4 py-2 rounded-lg font-medium text-sm transition whitespace-nowrap
    ...
    return (
      ...
      >
        ...
        <div className="flex flex-col items-center mt-8 mb-3">
          {/* Avatar con indicadores - CLICKEABLE */}
          <button
          ...
          {/* Días transcurridos - Solo Pendientes y Rechazadas */}
...
{inv.status === 'aceptada' ? (
  <div className="space-y-2">
    ...
    {/* Redes sociales en FILA - Compactas */}
  ...
  <div className="bg-white/50 rounded-lg p-3 text-center">
    <p className="text-xs text-gray-500">
          ...
          {filteredInvitations.length === 0 && (
            <div className="bg-white rounded-2xl shadow-lg p-12 text-center border-2 border-gray-200">
              ...
              <p className="text-gray-600">
                {filterStatus === 'todas' && 'Aún no has enviado ninguna invitación'}
      ...
      {/* MODAL ENVIAR MENSAJE - IGUAL QUE ANTES */}
